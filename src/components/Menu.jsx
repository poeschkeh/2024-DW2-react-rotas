import { IconBrandReact, IconHome, IconAddressBook, IconInfoCircle, IconSquareRoundedNumber1, IconSquareRoundedNumber2, IconSquareRoundedNumber3, IconSquareRoundedNumber4, IconMessageDown, IconMessageUp, } from "@tabler/icons-react";
...
export function Menu() {
  ...
  const getEstilo = (props) => {
    ...
    let estilo = "flex items-center gap-4 px-3 py-3 w-full text-sm text-white hover:bg-zinc-900";
  ...
  return (
    <aside className="flex flex-col gap-5 bg-black min-w-72">
      <header
        className={"flex justify-center items-center gap-2 bg-black text-zinc-50 px-1 py-5 h-16 text-2xl font-black"}
      >
        <IconBrandReact />
      ...
      >
        ...
        <NavLink to="ComunicacaoDireta" className={getEstilo}>
          <IconMessageDown/>
          Comunicação Direta</NavLink>

        <NavLink to="ComunicacaoIndireta" className={getEstilo}>
          <IconMessageUp/>
          Comunicação Indireta</NavLink>
